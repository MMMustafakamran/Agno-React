...
and an [AG-UI middleware](/agno/agentic-protocols/ag-ui-middleware) that applies it.
...
[`selfManagedAgents` or the local development option](/agno/backend/self-managed-agents).
...
- [AG-UI Middleware](/agno/agentic-protocols/ag-ui-middleware): the middleware
  lifecycle and the `Middleware` base class.
- [Self-managed agents](/agno/backend/self-managed-agents): passing your own agent
  instances to the frontend provider.
- [AgentRunner and persistence](/agno/backend/agent-runner): where the runtime keeps
  thread history.
