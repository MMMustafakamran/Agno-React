...
The video also shows [User Memory](/agno/intelligence/memories), then an expense-review workflow that becomes a skill.
...
For agent setup, use the [Mastra](/agno/intelligence/learned-skills#mastra), [LangGraph TypeScript](/agno/intelligence/learned-skills#langgraph-typescript), or [LangGraph Python](/agno/intelligence/learned-skills#langgraph-python) example. The delivery guide also covers [Google ADK](/agno/intelligence/learned-skills#google-adk), [BuiltInAgent](/agno/intelligence/learned-skills#builtinagent), and [Microsoft Agent Framework](/agno/intelligence/learned-skills#microsoft-agent-framework).
...
<Steps>
  ...
  <Step>
    ...
    For manual setup, follow the [framework adapter examples](/agno/intelligence/learned-skills#native-setup). Pick the adapter for the agent you already run. Configure that adapter in the agent server environment with the same project key and container ID used above.
    ...
    Keep the key server-side. Leave `CPK_INTELLIGENCE_SKILLS_REVISION` unset to follow the latest published Skills. See [container configuration](/agno/intelligence/learned-skills#configure-one-container) for self-hosted endpoints and exact revision pins.
  </Step>
...
| Delivery status is unavailable              | Select **Retry delivery status**. For self-hosted deployments, verify the delivery endpoint and migration are installed before relying on an adapter. See [deployment requirements](/agno/intelligence/learned-skills#deployment-requirements). |
