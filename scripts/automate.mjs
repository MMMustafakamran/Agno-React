...
// Check URL health with polling and fallback
async function waitForHealth(urls, name, timeoutMs = 45000) {
  ...
  throw new Error(`Timeout waiting for ${name} at ${urlList.join(' / ')}`);
}
...
function getPackageVersions() {
  ...
  try {
    ...
    versions.frontend = {
      ...
      '@ag-ui/agno': pkg.dependencies?.['@ag-ui/agno'] || 'n/a',
  ...
  try {
    ...
    versions.backend = {
      ...
      'agno': pyproject.match(/["']agno([^\s"']*)["']/)?.[0] || 'n/a',
      'ag-ui-protocol': pyproject.match(/["']ag-ui-protocol([^\s"']*)["']/)?.[0] || 'n/a',
      'fastapi': pyproject.match(/["']fastapi([^\s"']*)["']/)?.[0] || 'n/a',
      'openai': pyproject.match(/["']openai([^\s"']*)["']/)?.[0] || 'n/a',
    };
  } catch {
    // ignore
  }
...
function generateReport(data) {
  ...
  const lines = [];
  lines.push('# 📊 CopilotKit & Agno Automation & Recording Report\n');
  ...
  lines.push(`- **Backend AgentOS (\`:8000/status\`):** ${report.healthChecks.backend ? `✅ Healthy (${report.healthChecks.backend}s)` : '❌ Offline'}`);
...
async function main() {
  ...
  try {
    ...
    const backendHealth = await waitForHealth(
      ['http://127.0.0.1:8000/status', 'http://127.0.0.1:8000/docs', 'http://127.0.0.1:8000/'],
      'Backend AgentOS',
      45000,
    );
