...
export class RecordingEngine {
  ...
  async recordPage(config: PageRecordConfig): Promise<RecordResult> {
    ...
    try {
      ...
      // Everything from here to the end of the demo is what a docOnly page
      // does not have: no source worth showing, no dev server, no route to
      // drive. Its take is the doc read on camera, and that finished above.
      if (!config.docOnly) {
      // ----------------------------------------------------
