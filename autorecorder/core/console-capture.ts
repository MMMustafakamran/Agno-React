 ...
 * root (single-endpoint mode) or to `.../agent/<id>/...`. A thread-list or
 * inspector GET is not the run, and `ERR_ABORTED` is the browser cancelling a
 * request it no longer wants (`useThreads` refetching when its options change),
 * not a failure -- matching either failed healthy threads/learning pages.
 */
