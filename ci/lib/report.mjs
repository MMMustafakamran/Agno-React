...
function getPackageVersions() {
  ...
  try {
    ...
    // The version specifier is required in the match. Without it `agno` also
    // matched `name = "agno-copilotkit-backend"` on line 2 and the report
    // printed the project's own name as the library version.
    const dep = (name) =>
      pyproject.match(new RegExp(`["']${name}\\s*([<>=~!][^"']*)["']`))?.[1]?.trim() || 'n/a';
    versions.backend = {
      'requires-python': pyproject.match(/requires-python\s*=\s*"([^"]+)"/)?.[1] || 'n/a',
      agno: dep('agno'),
      'ag-ui-protocol': dep('ag-ui-protocol'),
      fastapi: dep('fastapi'),
      openai: dep('openai'),
    };
  } catch {
...
export function generateReport(data) {
  ...
  lines.push(
    `- **Backend AgentOS (\`:${BACKEND_PORT}/status\`):** ${
