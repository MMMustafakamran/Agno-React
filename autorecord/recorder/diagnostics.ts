...
/** Pre-flight check to verify if Next.js (port 3000) and Agno AgentOS (port 8000) are running */
...
export function diagnoseError(error: unknown, context?: string): string {
  ...
  if (errStr.includes('ECONNREFUSED') || errStr.includes('Failed to fetch')) {
    if (errStr.includes('8000') || context?.includes('backend')) {
      return (
        '🔴 [Agno Backend Offline]: The Python backend on port 8000 is not reachable.\n' +
        '   👉 Fix: Open a terminal, run: `cd backend && uv run main.py`'
      );
    }
