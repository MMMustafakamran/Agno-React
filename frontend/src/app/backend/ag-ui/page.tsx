    ...
    <>
      ...
      <Panel title="What it demonstrates">
        <p className="text-sm leading-relaxed text-slate-700 dark:text-slate-300">
          AG-UI is the event protocol underneath every CopilotKit integration.
          Messages, tool calls, state updates, and lifecycle transitions all
          travel as discrete events over SSE, and{" "}
          <code>agent.subscribe()</code> lets you observe them directly — which
          is the fastest way to tell whether a problem is in the agent, the
          runtime, or the UI.
        </p>
        <div className="mt-4">
          <TryIt
            prompts={["What's the weather in Tokyo?"]}
            expect="RUN_STARTED, a burst of TEXT_MESSAGE_CONTENT, TOOL_CALL_START/END around get_weather, TOOL_CALL_RESULT, then RUN_FINISHED."
      ...
      <Panel title="The proxy pattern">
        <p className="text-sm leading-relaxed text-slate-700 dark:text-slate-300">
          The browser never opens this stream against Agno. CopilotKit discovers
          agents through the runtime&apos;s <code>info</code> endpoint and gives
          each one a proxy implementing the same <code>AbstractAgent</code>{" "}
          interface. The runtime resolves the agent, clones it for request
          isolation, executes it, and re-encodes its events as SSE — so the
          frontend contract stays identical no matter which framework is behind
          it.
        </p>
      </Panel>
    </>
